            ...
            <div class="col-lg-6">
              ...
              <div class="row m-sm-0">
                ...
                <div class="col-sm-10 order-1 order-sm-2">
                  <div><a class="d-block" href="${producto.foto}" data-lightbox="product-${producto.id}" title="${producto.nombre}"><img class="img-fluid" src="${producto.foto}" alt="${producto.nombre}"></a></div>
                </div>
              </div>
            </div>
                ...
                <div class="col-sm-5 pr-sm-0">
                  <div class="border d-flex align-items-center justify-content-between py-1 px-3 bg-white border-white"><span class="small text-uppercase text-gray me-4 no-select">CANTIDAD</span>
                    <div class="quantity">
                      ...
                      <input id="cantidad-detalle-${producto.id}" class="form-control border-0 shadow-0 p-0" type="text" value="1">
                      <button class="inc-btn p-0"><i class="fas fa-caret-right"></i></button>
                    </div>
                  </div>
                </div>
                <div class="col-sm-3 pl-sm-0"><a id="agregarProducto-${producto.id}-detalle" class="btn btn-dark btn-sm btn-block h-100 d-flex align-items-center justify-content-center px-0">Agregar al carrito</a></div>
              </div><a class="btn btn-link text-dark p-0 mb-4" href="#"><i class="far fa-star me-2"></i>Añadir a favoritos</a><br>
              <ul class="list-unstyled small d-inline-block">
                <li class="px-3 py-2 mb-1 bg-white"><strong class="text-uppercase">id: </strong><span class="ms-2 text-muted">${producto.id}</span></li>
                <li class="px-3 py-2 mb-1 bg-white text-muted"><strong class="text-uppercase text-dark">Categoria: </strong><a class="reset-anchor ms-2" href="#">${producto.grupo}</a></li>
              </ul>
            </div>`;
...
const getData = async ()=>{
  ...
  let idProd = Number(localStorage.getItem("detalle-id"));
  let producto = (await getProductByIdFromAPI(idProd)).product;
